...
public sealed class RocketPlayer : IPlayer
{
    ...
    // _player can be null: UnturnedPlayer.FromPlayer returns null for players Rocket
    // hasn't wrapped yet (very early in the connect sequence).
    public CSteamID SteamId => _isConsole || _player == null ? CSteamID.Nil : _player.CSteamID;
    public string DisplayName => _isConsole ? "Console" : _player?.DisplayName ?? "<unknown>";
    public Vector3 Position => _isConsole || _player == null ? Vector3.zero : _player.Position;
