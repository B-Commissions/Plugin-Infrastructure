...
internal class MessageRemoveCommand : SubCommand
{
    public override string Name => "remove";
    public override string Permission => "zone.message.remove";
    public override string Help => "Remove an enter or leave message from a zone.";
    ...
    public override async Task Execute(IRocketPlayer caller, string[] args)
    {
        if (args.Length < 2)
        {
            CommandBase.Reply(caller, $"Usage: /zone message remove {Syntax}", Color.yellow);
            return;
        }
        ...
        {
            CommandBase.Reply(caller, $"No {args[1]} message on zone '{args[0]}'.", Color.red);
            return;
        }

        await ZonesPlugin.Instance.ZoneManager.SaveZoneAsync(zone);
        CommandBase.Reply(caller, $"{args[1]} message removed from zone '{args[0]}'.", Color.green);
    }
}
